...
class DrawingApp:
    ...
    #     predict(image_data)
    def predict_image(self):
        image_vector = np.array(self.image).reshape(1, -1) / 255.0
        """Send the 1×784 float vector to the FastAPI server and show result."""
        payload = {"image": image_vector.flatten().tolist()}
        try:
            API_URL = "http://127.0.0.1:8000/predict" 
            r = requests.post(API_URL, json=payload, timeout=5)
            r.raise_for_status()
            digit = r.json()["prediction"]
            messagebox.showinfo("Result", f"Digit: {digit}")
